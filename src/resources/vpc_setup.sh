...
SG_ID=$(aws ec2 create-security-group --group-name MySecurityGroup --description "Security group for public access" --vpc-id $VPC_ID --region $REGION --query 'GroupId' --output text)
echo "Created Security Group: $SG_ID"

# Allow inbound SSH and HTTP access
aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol tcp --port 22 --cidr 0.0.0.0/0 --region $REGION
echo "Allowed SSH (port 22) access from anywhere"

aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol tcp --port 80 --cidr 0.0.0.0/0 --region $REGION
echo "Allowed HTTP (port 80) access from anywhere"
